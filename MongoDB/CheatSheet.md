...
mongod --quite                             receive less notifications
mongod --dbpath arg                        DB locations
mongod --logpath [path]/logs.log           logs locations
mongod --help                              get help

storage options
mongod --repair                            run repair on all dbs
mongod --derectoryperdb                    store each db in a separate folder
mondod -f [path of config file]            starts the mongodb server with the configs in the file
mondod -config [path of config file]       starts the mongodb server with the configs in the file (the same as f)

mongo --help                               shows the help only for client not the server
...
## Basic Mongo DB config file:



# References:

Thanks to all those who offer their knoledge online.

[MongoDB quickstart](https://developer.mongodb.com/quickstart/cheat-sheet)
